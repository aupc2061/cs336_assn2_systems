...
import time
import wandb
from cs336b.layers import TransformerLM, softmax
from cs336b.optimizer import AdamW, lr_cosine_schedule
from  cs336b.utils import gradient_clipping, save_checkpoint
from cs336b.data import get_batch
from cs336b.losses import cross_entropy_loss
from cs336b.tokenizer import Tokenizer
...
def train(rank: int, world_size: int, data_path: str, val_data_path: str, vocab_path, merges_path, d_model, d_ff, num_heads, num_layers, context_length, theta, lr = 1e-3, num_steps: int = 10, device: str = "cpu", batch_size:int = 32, experiment_name: str = "baseline"):
    ...
    for step in range(num_steps):
        ...
        # Synchronize random seed across ranks so all get the same batch
        torch.manual_seed(42 + step)
        x, y = get_batch(tokens, batch_size, context_length, device)
        xr = x[rank::world_size]
        yr = y[rank::world_size]
        pred = model(xr)
        loss = cross_entropy_loss(pred, yr)
        optimizer.zero_grad()
        loss.backward()
        ...
        comm_fraction = comm_time / step_time if step_time > 0 else 0
...
def main():
    ...
    mp.spawn(
        ...
        args=(
            world_size,
            r"D:\CS336\assignment1-basics\data\TinyStoriesV2-GPT4-train.txt",
            r"D:\CS336\assignment1-basics\data\TinyStoriesV2-GPT4-valid.txt",
            "cs336b/bpe_vocab.pkl",
            "cs336b/bpe_merges.pkl",
            256,
            1024,
            8,
            6,
